...
function normalizeInvoiceId(id, invoice, legacyIndex) {
    ...
    const legacySeed = JSON.stringify({
        ...
        clientPhoneHash: typeof invoice.clientPhoneHash === "string"
            ? String(invoice.clientPhoneHash)
            : undefined,
        clientEmail: invoice.clientEmail?.trim() || "",
        clientGST: invoice.clientGST?.trim() || "",
        clientGstHash: typeof invoice.clientGstHash === "string"
            ? String(invoice.clientGstHash)
            : undefined,
        customerIdentityKey: typeof invoice.customerIdentityKey === "string"
            ? String(invoice.customerIdentityKey)
            : undefined,
...
export function normalizeInvoiceRecord(invoice, legacyIndex = 0) {
    ...
    return {
        ...
        updated_at: typeof invoice.updated_at === "string" ? String(invoice.updated_at) : undefined,
        deleted_at: typeof invoice.deleted_at === "string" ? String(invoice.deleted_at) : undefined,
        sync_status: typeof invoice.sync_status === "string" ? String(invoice.sync_status) : undefined,
        last_synced_at: typeof invoice.last_synced_at === "string" ? String(invoice.last_synced_at) : undefined,
        invoiceNumber: invoice.invoiceNumber?.trim() || "",
        ...
        resetMonthDayAtCreation: typeof invoice.resetMonthDayAtCreation === "string" && invoice.resetMonthDayAtCreation.trim()
            ...
            : null,
        sequenceWindowStart: typeof invoice.sequenceWindowStart === "string" && invoice.sequenceWindowStart.trim()
            ? invoice.sequenceWindowStart.trim()
            : null,
        sequenceWindowEnd: typeof invoice.sequenceWindowEnd === "string" && invoice.sequenceWindowEnd.trim()
...
export function normalizeInvoiceRecords(invoices) {
    if (!Array.isArray(invoices)) {
        return { invoices: [], changed: false };
    }
    let changed = false;
...
export function normalizeInvoiceStorePayload(payload) {
    let changed = false;
...
export function serializeInvoiceStore(invoices) {
...
export function readStoredInvoices() {
    ...
    const { getActiveOrGlobalItem, setActiveOrGlobalItem } = require("@/lib/userStore");
...
export function writeStoredInvoices(invoices) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { setActiveOrGlobalItem } = require("@/lib/userStore");
    setActiveOrGlobalItem("invoices", serializeInvoiceStore(invoices));
}
export function getStoredBusinessRecord() {
    if (typeof window === "undefined") {
        return null;
    }
    // eslint-disable-next-line @typescript-eslint/no-require-imports
