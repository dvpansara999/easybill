...
export function normalizeResetMonthDay(value) {
    if (!value)
        return DEFAULT_RESET_MONTH_DAY;
...
export function formatResetMonthDayLabel(value) {
...
export function formatResetMonthLabel(value) {
    const normalized = normalizeResetMonthDay(value);
    const [monthRaw] = normalized.split("-");
    const month = Number(monthRaw);
    return MONTH_LABELS[month - 1];
}
export function getResetMonthDayOptions() {
    const options = [];
