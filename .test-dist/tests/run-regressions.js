...
import { INVOICE_SCHEMA_VERSION, normalizeInvoiceStorePayload, replaceInvoiceById, serializeInvoiceStore, validateInvoiceRecord, } from "../lib/invoice.js";
...
import { activeRecords, ensureRecordIds, markRecordDeleted, mergeActiveWithExistingTombstones, } from "../lib/workspaceTombstones.js";
import { redactSensitiveData } from "../lib/workspaceSecurity.js";
import { validateCustomerForPersistence, validateGstin, validateProductForPersistence, } from "../lib/workspaceValidation.js";
...
runCase("Server workspace setup writes surface Supabase errors", () => {
    ...
    const profileSource = sealingSource.slice(sealingSource.indexOf("export async function upsertSealedProfileFromCache"), sealingSource.indexOf("export async function upsertSealedCustomersFromCache"));
    const seedSource = sealingSource.slice(sealingSource.indexOf("export async function ensureSealedWorkspaceSeed"), sealingSource.indexOf("export async function backfillSealedWorkspace"));
    const settingsSource = repositorySource.slice(repositorySource.indexOf("export async function upsertSettingsPatch"), repositorySource.indexOf("export async function upsertProductsFromCache"));
...
runCase("incremental sync marks unsynced and newer local records as dirty", () => {
    assert.equal(isLocalRecordDirty({
        id: "prod_1",
...
runCase("incremental sync plan pushes dirty local records and applies changed remote rows", () => {
    ...
    assert.deepEqual(plan.localToPush.map((row) => row.id), ["prod_local"]);
    assert.deepEqual(plan.remoteToApply.map((row) => row.id), ["prod_remote", "prod_new_remote"]);
    assert.deepEqual(plan.conflicts.map((row) => [row.id, row.winner]), [["prod_remote", "remote"]]);
});
...
runCase("Supabase clients normalize project urls before auth initialization", () => {
    assert.equal(normalizeSupabaseProjectUrl("https://example.supabase.co/rest/v1"), "https://example.supabase.co");
    assert.equal(normalizeSupabaseProjectUrl("https://example.supabase.co/auth/v1"), "https://example.supabase.co");
    assert.equal(normalizeSupabaseProjectUrl("https://example.supabase.co/storage/v1/"), "https://example.supabase.co");
    for (const path of [
...
runCase("incremental cache merge applies remote products and tombstones without full replacement", () => {
    const merged = JSON.parse(mergeProductsCache(JSON.stringify([
        {
            id: "prod_keep",
            name: "Keep",
            hsn: "1",
    ...
    assert.deepEqual(merged.map((row) => row.id).sort(), ["prod_keep", "prod_new"]);
});
runCase("incremental customer merge preserves existing rows while applying changed rows", () => {
    const merged = JSON.parse(mergeCustomersCache(JSON.stringify([{ id: "cust_keep", name: "Keep", phone: "1", gst: "", email: "", address: "" }]), [
        {
            id: "cust_new",
            name: "New",
            phone: "2",
    ...
    assert.deepEqual(merged.map((row) => row.id).sort(), ["cust_keep", "cust_new"]);
});
runCase("incremental invoice merge applies remote tombstones", () => {
    ...
    const merged = normalizeInvoiceStorePayload(JSON.parse(mergeInvoicesCache(JSON.stringify(existing), [
        {
            id: "inv_delete",
    ...
    assert.deepEqual(merged.store.invoices.map((invoice) => invoice.id), ["inv_keep"]);
});
        ...
        cache: {
            ...
            primeUser(id, entries) {
                ...
                for (const entry of entries)
                    cacheValues.set(`${id}:${entry.key}`, entry.value);
            },
            readWatermark() {
                return null;
            },
...
runCase("sync retry queue dedupes by user key and operation", () => {
    ...
    assert.deepEqual(queue.list("user_1").map((item) => item.rawValue), ["new"]);
});
runCase("sync retry queue keeps separate push and delete operations and tracks failures", () => {
    ...
    queue.remove(del.id);
    assert.deepEqual(queue.list("user_1").map((item) => item.operation), ["push"]);
});
...
runCase("workspace tombstones hide deleted records while preserving deletion intent", () => {
    ...
    assert.deepEqual(activeRecords(deleted).map((row) => row.name), ["Two"]);
...
runCase("active cache writes keep existing tombstones alongside visible records", () => {
    ...
    assert.deepEqual(merged.map((row) => row.id), ["prod_deleted", "prod_keep"]);
...
await runCaseAsync("authoritative cloud save failure leaves cache untouched", async () => {
    let cached = '[{"id":"prod_old","name":"Old"}]';
    const dataAccess = createWorkspaceDataAccess({
        cache: {
            get() {
                return cached;
    ...
    await assert.rejects(() => dataAccess.saveProduct('[{"id":"prod_new","name":"New"}]'), /network down/);
